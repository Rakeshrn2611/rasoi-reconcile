...
export default function GiftCards({ selectedVenue, venues, showToast }) {
  ...
  function doExport() {
    downloadCSV('gift-cards.csv', rows.map(r => ({
      Date: r.date, Venue: r.venue_name, 'Card Last 4': r.gift_card_last4,
      'Receipt #': r.receipt_number, Type: r.activity_type, Amount: f2(r.amount),
    })));
  }
        ...
        {loading ? <p style={s.empty}>Loading…</p>
         ...
         : isMobile ? rows.map((r,i) => (
             <MobileCard key={i} title={fDate(r.date)} subtitle={r.venue_name} items={[
               ...
               { label: 'Type',   value: r.activity_type },
               { label: 'Amount', value: `£${f2(r.amount)}`, strong: true, color: '#2563eb' },
             ]} />
           ))
         : (
           <table style={s.table}>
             <thead><tr>{['Date','Venue','Card (Last 4)','Receipt #','Type','Amount'].map(h=><th key={h} style={s.th}>{h}</th>)}</tr></thead>
             <tbody>{rows.map((r,i)=>(
               <tr key={i} style={{background:i%2===0?'#fff':'#fefcf9'}}>
                 ...
                 <td style={s.td}><Tag text={r.activity_type}/></td>
                 <td style={{...s.td,fontWeight:700,color:'#2563eb'}}>£{f2(r.amount)}</td>
               </tr>
             ))}</tbody>
           </table>
         )}
