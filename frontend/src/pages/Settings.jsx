...
export default function Settings({ venues, showToast, refreshVenues }) {
  ...
  return (
    <div style={{ ...s.root, maxWidth: isMobile ? '100%' : 700 }}>
      <div style={s.card}>
        ...
        <div style={s.venueList}>
          {venues.map(v => <VenueRow key={v.id} venue={v} showToast={showToast} onSave={refreshVenues} isMobile={isMobile} />)}
        </div>
      </div>
...
function VenueRow({ venue, showToast, onSave, isMobile }) {
  ...
  return (
    <div style={{ ...s.venueRow, flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center' }}>
      <div style={s.venueMark}>{name[0]}</div>
      <div style={{ ...s.venueFields, flexDirection: isMobile ? 'column' : 'row' }}>
        <input value={name} onChange={e => setName(e.target.value)}
          style={{ ...s.input, fontSize: isMobile ? 16 : 13 }} placeholder="Venue name" />
        <input value={locationId} onChange={e => setLocationId(e.target.value)}
          style={{ ...s.input, fontFamily: 'monospace', fontSize: isMobile ? 14 : 12 }}
          placeholder="Square Location ID (e.g. LXXXXXXXXXXXXXXXX)" />
      </div>
      <button onClick={save} disabled={saving}
        style={{ ...s.saveBtn, padding: isMobile ? '12px 24px' : '8px 16px', fontSize: isMobile ? 15 : 13 }}>
        {saving ? '…' : 'Save'}
      </button>
    </div>
  );
...
const s = {
  ...
  venueRow:     { display: 'flex', gap: 12, padding: '12px 0', borderBottom: '1px solid #f5ede0' },
  ...
  saveBtn:      { padding: '8px 16px', background: '#c1440e', color: '#fff', border: 'none', borderRadius: 7, fontSize: 13, fontWeight: 600, cursor: 'pointer', alignSelf: 'flex-start' },
